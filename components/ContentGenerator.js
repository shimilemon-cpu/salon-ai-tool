...
export default function ContentGenerator({ apiKey, onChangeKey }) {
  ...
  const generateOne = async (tabId) => {
    ...
    try {
      ...
      setResults(p => ({ ...p, [tabId]: parsed }));
      if (tabId === "hotpepper" && parsed.tags) {
        const tagData = { styleTitle: parsed.styleTitle || "", tags: parsed.tags };
        window.postMessage({ type: "SALON_AI_HPB_TAGS", ...tagData }, "*");
        try { localStorage.setItem("latestHpbTags", JSON.stringify(tagData)); } catch {}
        fetch("/api/latest-tags", {
          method: "POST",
